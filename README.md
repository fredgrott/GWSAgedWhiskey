...
While RxJava certainly solves some things to use it effectively one should be using dagger and some other
things which makes rxJava's steep learning curve even more steep. So this is a MVP library not using
rxjava and not using dagger or using Google's data binding to make things easier.

You can use this library with my GWSBindroid library, which is a binding library.
...
```groovy
compile 'com.github.shareme:GWSAgedWhiskey:{latest-release-number}@aar'
```
